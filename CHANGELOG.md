...
- **GHCR Docker image** — CI copies `.next/standalone` with `cp -a` (plain `cp -r` on `standalone/*` followed symlinks and packed the whole repo; runtime lacked `.next/BUILD_ID`); `Dockerfile.ci` copies from `.next/standalone` and asserts `BUILD_ID` / no leaked repo files
