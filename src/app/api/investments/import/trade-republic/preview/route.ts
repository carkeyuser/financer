...
export async function POST(request: Request) {
  ...
  if (!(file instanceof File)) {
    return Response.json({ error: "Keine CSV-Datei hochgeladen" }, { status: 400 })
  }
  ...
  if (targetUserIdRaw && targetUserIdRaw !== ctx.userId) {
    ...
    if (!targetMember) {
      return Response.json({ error: "Zielbenutzer nicht im Haushalt" }, { status: 400 })
    }
  ...
  try {
    ...
    return Response.json({ error: messages[code] ?? "CSV konnte nicht gelesen werden" }, { status: 400 })
  }
  ...
  return createNdjsonResponse(async (emit) => {
    ...
    const yahooResolutions = await resolveIsins(
      ...
      (current, total) => emit({ type: "progress", phase: "tickers", current, total })
    )

    emit({ type: "progress", phase: "database", current: 0, total: 1 })
    ...
    emit({ type: "progress", phase: "database", current: 1, total: 1 })
    emit({ type: "progress", phase: "dedup", current: 0, total: 1 })
    ...
    emit({ type: "progress", phase: "dedup", current: 1, total: 1 })
