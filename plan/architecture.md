...
**Provisionierte Benutzer:** `User.provisionedByUserId` wird bei `POST /api/admin/users` gesetzt. Owner und Haushalts-Admin sehen Tenant-User, die nicht im aktiven Haushalt sind, unter **Benutzer → Angelegte Benutzer (eigene Haushalte)** (`GET /api/household` → `provisionedUsers`, gefiltert nach Haushalts-Owner). Bearbeiten/2FA/Löschen über `PATCH|DELETE /api/admin/users/[userId]` und `PATCH /api/admin/2fa/[userId]` — Berechtigung: Owner selbst oder Admin des Haushalts, dessen Owner den Tenant angelegt hat. `DELETE` entfernt Benutzer inkl. Einpersonen-Tenant-Haushalt (`deleteProvisionedUserAccount`).
